...
public class ClickHouseDatabase extends Database<ClickHouseConnection> {
    ...
    public void useSchema(String schema) throws SQLException {
        ...
        String currentSchema = getMainConnection().getCurrentSchemaNameOrSearchPath();

        if (!currentSchema.equalsIgnoreCase(schema)) {
            getMainConnection().doChangeCurrentSchemaOrSearchPathTo(schema);
        }
    }

    public void restoreOriginalSchema() throws SQLException {
        getMainConnection().doChangeCurrentSchemaOrSearchPathTo(getMainConnection().getOriginalSchema());
    }
